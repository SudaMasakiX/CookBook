<template>
  <view class="min-h-screen bg-gray-50 pb-safe-bottom">
    <view v-if="loading" class="p-4">
      <view class="skeleton h-64 mb-4 rounded-lg"></view>
      <view class="skeleton h-8 w-3/4 mb-2 rounded"></view>
      <view class="skeleton h-4 w-full mb-4 rounded"></view>
    </view>

    <!-- 表单容器 -->
    <view v-else class="mx-4 py-4 space-y-4">
      <!-- 食谱图片 -->
      <view class="card">
      ...
      <view class="card">
        ...
        <view class="flex flex-wrap">
          ...
          >
            {{ tag }}
            <text class="ml-1 text-red-500 cursor-pointer" @click="removeTag(index)">×</text>
          </view>
        </view>
        <view class="mt-2">
...
const loadRecipe = async (id) => {
  try {
    ...
    if (result.success) {
      ...
      form.difficulty = recipe.difficulty.toString()
      form.cookingTime = recipe.cookingTime.toString()
      form.servings = recipe.servings.toString()
      form.tags = [...recipe.tags]
      form.ingredients = recipe.ingredients.length > 0 ? [...recipe.ingredients] : [{ name: '', amount: '' }]
      form.steps = recipe.steps.length > 0 ? [...recipe.steps] : [{ description: '', images: [] }]
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const result = await recipeAPI.update(recipeId.value, recipeData)
    
    if (result.success) {
      appStore.updateRecipe(recipeId.value, result.data)
